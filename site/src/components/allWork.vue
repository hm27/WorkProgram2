    ...
    <div class="wrap">
        ...
        <div class="wrap-aWork" v-for="item in wrokList">
            <div class="aWork-top" >
                ...
                <p>{{item.aPost}}</p>
                <p>
                    <img src="../assets/more.gif" alt="" @click="$router.push('/allDayList')">
                </p>
                <p></p>
            </div>
            <div class="wrok-det">
                ...
                <p>工作天数:{{item.aDays}} 天</p>
                <p style="white-space: pre-wrap;">工作内容:{{item.aContent}}</p>
                <p>联系人电话:{{item.aNumber}}</p>
                <p>招聘人数:{{item.aRecruits}}</p>
            </div>
            <div class="btn"><button @click="openPost(item._id,item.aPost,item.aContent,item.aTime,item.aTime2,item.aSalary,item.aDays)">申请岗位</button></div>
        </div>
        <div><!--关于申请岗位信息的填写-->
            <el-dialog title="确认申请信息" :visible.sync="FormVisible">
                <el-form :model="form"  :rules="rules" ref="form">
                    ...
                    <el-form-item label="工作天数" :label-width="formLabelWidth" prop="Days">
                        <el-input ref="aDays" v-model="form.aDays" auto-complete="off"></el-input>
                    </el-form-item>
...
<script>
    export default {
            ...
            return{
                ...
                rules:{
                    aWrokexp:[
                        ...
                        { pattern: /^(?!(\d+)$)[\u4e00-\u9fffa-zA-Z\d\-_]+$/, message: '请输入正确的工作经验' }
                    ],
                    phone:[
                        { required: true, message: '请输入手机号', trigger: 'blur' },
                        { pattern: /^1[34578]\d{9}$/, message: '请输入正确的手机号11位' }
                    ]
                }
            }
        },
        methods:{
            ...
            openPost(id,aPost,aContent,aTime,aTime2,aSalary,Days){/*打开弹窗，并把数据给上去*/
                this.workId = id;
            ...
            submitForm(formName){/*申请岗位*/
                this.$refs[formName].validate((valid) =>{
                    if (valid) {
                        ...
                        this.$message.error('请输入正确的工作经验或手机号! ');
                        return false;
                    }
                });
            },
