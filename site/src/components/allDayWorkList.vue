<template>
    <div>
        <div class="work-wrap">
            <div class="ji" v-for="item in allDayWorkList">
                <div class="ji-fl">
                    <a href="" class="ji-item-info-jobName">{{item.aPost}}</a>
                    <p class="ji-p">薪资: {{item.aSalary}}/天</p>
                    <a href="" class="ji-item-info-companyName">{{item.aContent}}</a>
                </div>
                <div class="ji-fr">
                    <p>{{item.aWhere}}</p>
                    <div>
                        <span>{{item.aTime | date}} - {{item.aTime2 | date}}</span>
                    </div>
                    <button class="btn" @click="openPost(item._id,item.aPost,item.aContent,item.aTime,item.aTime2,item.aSalary)">申请岗位</button>
                </div>
            </div>
            <div class="foot">
                勤工助学管理系统
...
<script>
    export default {
        ...
        methods:{
            getAllDayWorkList(){/*获取全天工工作列表前端*/
                this.$ajax.get("/getAllDayWorkList")
                    .then(data=>{
                        if(data.ok === 1){
                            this.allDayWorkList = data.allDayWorkInfo
                        }
                    })
            },
